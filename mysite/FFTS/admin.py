...
# first way to disable delete site-wide action
...
def allow_anonymous_user():
    ...
    AnonymousUser.is_superuser = False
...
class ProjectAdmin(AjaxAdmin):
    ...
    """
        if len(queryset) == 1 and 'FFTS0000000000_SYNC_CONFIG' == queryset[0].ffts_id:
            count = 0
            ...
            self.message_user(request, 'ONLY FFTS0000000000_SYNC_CONFIG CAN BE SELECTED!', messages.WARNING)
    ...
    """
        count = 0
        for obj in queryset:
            if 'FFTS0000000000_SYNC_CONFIG' == obj.ffts_id:
                self.message_user(request, 'FFTS0000000000_SYNC_CONFIG IGNORED!', messages.WARNING)
            else:
                configfile = settings.CONFIGDIR + obj.ffts_id + '.xml'
                if Path(configfile).is_file():
                    sync_from_xml(configfile, obj)
